...
export function CMSPage() {
  ...
  return (
    <div className="fade-in">
      <PageHeader
        title="CMS Content"
        subtitle="Manage website pages and banners"
        action={
          <Btn
            onClick={() => {
              setForm({ key: "", title: "", content: "", type: "page" });
              setModal(true);
            }}
            icon={<Plus size={15} />}
          >
      ...
      {content.length === 0 ? (
        <Empty
          ...
          desc="Add pages, banners and FAQs."
          action={<Btn onClick={() => setModal(true)}>Add Content</Btn>}
        />
      ) : (
        <div
          style={{
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            ...
            <textarea
              ...
              value={form.content}
              onChange={handle}
              placeholder="Page or banner content..."
              rows={5}
          ...
          <FormActions>
            ...
            <Btn type="submit" loading={saving} icon={<Save size={15} />}>
              Save Content
            </Btn>
          </FormActions>
        </form>
...
export function SubAdminPage() {
  ...
  const handlePerm = (e) =>
    setForm((p) => ({
      ...p,
      permissions: { ...p.permissions, [e.target.name]: e.target.checked },
    }));
  ...
  return (
    <div className="fade-in">
      <PageHeader
        ...
        action={
          <Btn onClick={() => setModal(true)} icon={<UserPlus size={15} />}>
            Add Sub Admin
          </Btn>
        }
      />
      ...
      <Alert
        type="info"
        message="Sub admins can only access features you grant them. Super admin has full access."
      />
              ...
              {[
                ...
                <label
                  ...
                  style={{
                    ...
                    padding: "8px 12px",
                    borderRadius: "8px",
                    background: form.permissions[p.name]
                      ? "var(--accent-dim)"
                      : "var(--bg3)",
                  }}
                >
                  <input
                    type="checkbox"
                    name={p.name}
                    checked={form.permissions[p.name]}
                    onChange={handlePerm}
                    style={{ width: "auto" }}
                  />
                  {p.label}
...
export function BulkMailPage() {
  ...
  const send = async (e) => {
    ...
    if (
      !confirm(
        `Send mail to ${form.targets === "all" ? "ALL restaurants" : "selected restaurants"}?`,
      )
    )
      return;
    setSending(true);
  ...
  return (
    <div className="fade-in">
      <PageHeader
        title="Bulk Mail"
        subtitle="Send notifications to all or selected restaurants"
      />
      ...
      <div style={{ maxWidth: "640px" }}>
        <Card>
          ...
          >
            <Alert
              type="warning"
              message="This will send real emails to restaurant admins. Double check before sending."
            />
